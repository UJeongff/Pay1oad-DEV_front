...
export default function AssignmentDetailPage() {
  ...
  const handleDeleteSubmission = async () => {
    ...
    const res = await fetchWithAuth(
      `${API_URL}/v1/contents/${contentId}/assignments/${assignmentId}/submissions/${mySubmission.id}`,
      { method: 'DELETE' }
    )
    if (!res.ok) {
  ...
  if (!assignment) return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(180deg, #0E111C 0%, #0D1530 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <p style={{ color: 'rgba(255,255,255,0.5)' }}>과제를 찾을 수 없습니다.</p>
    </main>
  )
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap' }}>
            <div>
              ...
              <div style={{ marginTop: '12px', display: 'flex', gap: '16px', flexWrap: 'wrap', fontSize: '12px', color: 'rgba(255,255,255,0.4)' }}>
                <span>작성자 | {assignment.authorName}</span>
                <span>작성일 | {formatDate(assignment.createdAt)}</span>
                {assignment.dueAt && <span style={{ color: new Date(assignment.dueAt) < new Date() ? '#ff9a9a' : 'rgba(255,255,255,0.4)' }}>마감 | {formatDate(assignment.dueAt)}</span>}
              </div>
            </div>
        ...
        {isLeaderOrAdmin ? (
          <section style={{ display: 'grid', gap: '12px' }}>
            ...
            {submissions.length === 0 && (
              <div style={{ padding: '20px', borderRadius: '12px', background: 'rgba(255,255,255,0.03)', color: 'rgba(255,255,255,0.45)' }}>아직 제출이 없습니다.</div>
            )}
                ...
                <div key={submission.id} style={{ borderRadius: '12px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', padding: '20px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap' }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                      ...
                      <button
                        onClick={() => { setGradingId(isOpen ? null : submission.id); setGradeStatus('O'); setGradeFeedback(submission.feedback ?? '') }}
                        style={{ padding: '6px 12px', borderRadius: '8px', border: '1px solid rgba(28,90,255,0.35)', background: 'transparent', color: '#8fb6ff', cursor: 'pointer' }}
                      >
                        {isOpen ? '닫기' : '평가'}
                      </button>
                    </div>
                  ...
                  {submission.files.length > 0 && (
                    <div style={{ marginTop: '12px', display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                      {submission.files.map(file => (
                        <a key={file.id} href={`${API_URL}${file.fileUrl}`} target="_blank" rel="noopener noreferrer" style={{ color: '#a9c5ff', textDecoration: 'none', fontSize: '12px' }}>
                    ...
                    <div style={{ marginTop: '16px', display: 'grid', gap: '10px' }}>
                      <div style={{ display: 'flex', gap: '8px' }}>
                        {(['O', 'LATE', 'X'] as const).map(value => (
                          <button key={value} onClick={() => setGradeStatus(value)} style={{ padding: '6px 12px', borderRadius: '999px', border: gradeStatus === value ? '1px solid rgba(28,90,255,0.6)' : '1px solid rgba(255,255,255,0.15)', background: gradeStatus === value ? 'rgba(28,90,255,0.15)' : 'transparent', color: gradeStatus === value ? '#8fb6ff' : 'rgba(255,255,255,0.5)', cursor: 'pointer' }}>{value}</button>
                        ))}
                      </div>
                      ...
                      <div>
                        <button onClick={() => handleGrade(submission.id)} disabled={grading} style={{ padding: '8px 16px', borderRadius: '8px', border: 'none', background: '#1C5AFF', color: '#fff', cursor: 'pointer' }}>
                          {grading ? '저장 중...' : '평가 저장'}
                        </button>
                      </div>
                    </div>
            ...
            {mySubmission ? (
              <>
                ...
                {mySubmission.feedback && (
                  <div style={{ marginTop: '16px', padding: '14px', borderRadius: '10px', background: 'rgba(28,90,255,0.08)', border: '1px solid rgba(28,90,255,0.2)' }}>
