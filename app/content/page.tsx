...
const SLOTS: Array<{ col: number; row: number; span: number; variant: Variant }> = [
  { col: 1, row: 1, span: 2, variant: 'blue' },
  { col: 2, row: 1, span: 1, variant: 'white' },
  { col: 3, row: 1, span: 1, variant: 'gray' },
  { col: 2, row: 2, span: 1, variant: 'gray' },
  { col: 3, row: 2, span: 1, variant: 'white' },
  { col: 1, row: 3, span: 1, variant: 'gray' },
  { col: 2, row: 3, span: 1, variant: 'white' },
  { col: 1, row: 4, span: 1, variant: 'white' },
  { col: 2, row: 4, span: 1, variant: 'gray' },
  { col: 3, row: 3, span: 2, variant: 'blue' },
]
...
function Badge({ label, icon, vs }: { label: string; icon?: React.ReactNode; vs: typeof VS[Variant] }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '11px', fontWeight: 600, padding: '3px 10px', borderRadius: '100px', background: vs.badgeBg, color: vs.badgeColor, border: `1px solid ${vs.badgeBorder}` }}>
      {icon}{label}
    </span>
  )
}
...
function ContentCard({
  ...
  return (
    <div style={{ ...gridPos(slot), position: 'relative' }}>
      <Link
        href={`/content/${content.id}`}
        style={{ display: 'flex', flexDirection: 'column', height: '100%', borderRadius: '16px', padding: isBlue ? '24px' : '20px', background: vs.bg, textDecoration: 'none', overflow: 'hidden', transition: 'transform 0.2s, box-shadow 0.2s', cursor: 'pointer' }}
        onMouseEnter={e => {
      ...
      >
        <h3 style={{ color: vs.title, fontSize: isBlue ? '1.4rem' : '1.2rem', fontWeight: 900, letterSpacing: '0.02em', textTransform: 'uppercase', lineHeight: 1.15, fontFamily: "var(--font-archivo-black), 'Archivo Black', sans-serif", flex: isBlue ? 1 : undefined, marginBottom: isBlue ? 0 : 'auto', paddingBottom: isBlue ? 0 : '12px', paddingRight: isAdmin ? '28px' : 0 }}>
          {content.title}
        </h3>
        ...
        <div>
          ...
          {content.description && (
            <p style={{ color: vs.text, fontSize: '12px', lineHeight: 1.65, display: '-webkit-box', WebkitLineClamp: slot.span === 2 ? 6 : 3, WebkitBoxOrient: 'vertical' as const, overflow: 'hidden' }}>
              {content.description}
            </p>
          )}
        </div>
...
function PlaceholderCard({ slot }: { slot: typeof SLOTS[number] }) {
  return (
    <Link
      href="/content/create"
      style={{ ...gridPos(slot), display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', borderRadius: '16px', border: '1.5px dashed rgba(255,255,255,0.18)', background: 'rgba(255,255,255,0.02)', textDecoration: 'none', cursor: 'pointer', transition: 'border-color 0.2s, background 0.2s' }}
      onMouseEnter={e => {
        const el = e.currentTarget as HTMLElement
      ...
      <section className="relative pb-20">
        <div className="relative max-w-5xl mx-auto px-[5vw]">
          <div className="lg:absolute lg:right-[5vw] lg:top-0 lg:z-10" style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '16px' }}>
            <Link
              href="/content/create"
              style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '8px 16px', borderRadius: '8px', border: '1.5px dashed rgba(255,255,255,0.3)', background: 'transparent', color: 'rgba(255,255,255,0.6)', fontSize: '13px', fontWeight: 500, textDecoration: 'none', transition: 'border-color 0.15s, color 0.15s' }}
              onMouseEnter={e => {
                const el = e.currentTarget as HTMLElement
          ...
          {totalPages > 1 && (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px', marginTop: '48px' }}>
              <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} style={{ width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: 'none', cursor: page === 1 ? 'default' : 'pointer', color: page === 1 ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.6)', borderRadius: '6px' }}>
              ...
              <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} style={{ width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'transparent', border: 'none', cursor: page === totalPages ? 'default' : 'pointer', color: page === totalPages ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.6)', borderRadius: '6px' }}>
                <svg width="7" height="12" viewBox="0 0 7 12" fill="none">
