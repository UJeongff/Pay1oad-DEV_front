...
type GoogleOAuthLogin = {
  signupStatus?: 'PENDING' | 'COMPLETED'
...
function LoginContent() {
  ...
  async function requestGooglePrecheck(credential: string): Promise<GoogleOAuthPrecheck> {
    const res = await fetch(`${API_URL}/v1/auth/google/precheck`, {
      method: 'POST',
    ...
    if (!res.ok) {
      throw new Error(data?.message ?? 'Google 로그인 사전 확인에 실패했습니다.')
    }

    return data?.data ?? {}
  }

  async function requestGoogleLogin(credential: string): Promise<GoogleOAuthLogin> {
  ...
  async function finishGoogleLogin(loginData: GoogleOAuthLogin) {
    if (loginData.signupStatus === 'PENDING') {
  ...
  async function handleGoogleCredential(response: { credential: string }) {
    ...
    try {
      const precheck = await requestGooglePrecheck(response.credential)

      if (precheck.requiresLinkConfirmation) {
        setPendingGoogleCredential(response.credential)
        setPendingGoogleEmail(precheck.email ?? '')
  ...
  async function handleConfirmGoogleLink() {
    ...
    try {
      const loginData = await requestGoogleLogin(pendingGoogleCredential)
  ...
  function handleCancelGoogleLink() {
    setShowGoogleLinkModal(false)
    setPendingGoogleCredential(null)
    setPendingGoogleEmail('')
  }
    ...
    <>
      ...
      {showGoogleLinkModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
          <div
            className="w-full max-w-md rounded-3xl border border-white/10 bg-[#10131a]/95 p-6 text-white shadow-2xl"
            style={{ backdropFilter: 'blur(18px)' }}
          >
            <p className="text-xs font-semibold tracking-[0.24em] text-blue-300/80">GOOGLE 연동 확인</p>
            <h2 className="mt-3 text-xl font-semibold leading-snug">기존 계정에 Google 로그인을 연결할까요?</h2>
            <p className="mt-4 text-sm leading-6 text-white/72">
              {pendingGoogleEmail || '선택한 이메일'}로 가입된 기존 계정이 있습니다. 계속하면 이 계정에 Google 로그인이 연결됩니다.
            </p>
            <p className="mt-2 text-sm leading-6 text-white/52">
              연동하지 않음을 선택하면 이번 Google 로그인은 진행되지 않습니다.
            </p>

            <div className="mt-6 flex gap-3">
              <button
                type="button"
                onClick={handleCancelGoogleLink}
                disabled={googleLoading}
                className="flex-1 rounded-2xl border border-white/12 px-4 py-3 text-sm font-medium text-white/80 transition hover:bg-white/6 disabled:cursor-not-allowed disabled:opacity-60"
              >
                연동하지 않음
              </button>
              <button
                ...
                disabled={googleLoading}
                className="flex-1 rounded-2xl bg-[#0041EF] px-4 py-3 text-sm font-semibold text-white transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-60"
              >
                {googleLoading ? '연동 중...' : '연동 후 계속'}
              </button>
            </div>
          </div>
        </div>
